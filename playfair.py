...
def generatekey(key):
    keyletters = []
    # make sure letters passed to key are not repeated
    for char in key:
...
def decrypt(splitted, key):
    ...
    for i in range(len(splitted)):
        x1, y1 = find(splitted[i][0], key)
        x2, y2 = find(splitted[i][1], key)
        # if in same row, move one space to the right
        if x1 == x2:
            y1 -= 1
            y2 -= 1
            # loop back to start if reaches edge
            ...
            if y2 == -1:
                y2 = 4
        # if in same column, move one space down
        elif y1 == y2:
            x1 -= 1
            x2 -= 1
            # loop back to start if reaches edge
